...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 max-w-[2000px] mx-auto">
      <div className="flex items-center justify-between bg-white border-b-4 border-b-black w-full px-4 lg:px-36">
          ...
          {menus.map(({ href, label }, index) => (
            <React.Fragment key={index}>
              <li className="h-full">
                <Link
                  href={href}
                  className={`flex items-center justify-center text-black w-28 h-14 font-medium ${
